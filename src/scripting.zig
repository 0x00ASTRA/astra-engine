...
fn getScreenWidth(lua: *Lua) c_int {
    const engine = getEngine(lua) catch |err| {
        std.debug.print("draw_texture error: could not get engine pointer: {s}\n", .{@errorName(err)});
        return 0;
    };

    const sw: i32 = engine.renderer.get_screen_width();
    lua.pushNumber(@floatFromInt(sw));
    return 1;
}

/// Get the height of the screen.
fn getScreenHeight(lua: *Lua) c_int {
    const engine = getEngine(lua) catch |err| {
        std.debug.print("draw_texture error: could not get engine pointer: {s}\n", .{@errorName(err)});
        return 0;
    };

    const sh: i32 = engine.renderer.get_screen_height();
    lua.pushNumber(@floatFromInt(sh));
    return 1;
}
